...
export type ChordLyricSegment = { chord: string; lyric: string };

/**
 * Split a space-aligned chord/lyric pair into wrap units.
 * Each unit keeps its chord above its lyric slice so a trailing E
 * wraps with its words instead of overflowing the viewport.
 */
export function splitChordLyric(chordLine: string, lyricLine: string): ChordLyricSegment[] {
	const chord = (chordLine ?? '').replace(/\r/g, '');
	const lyric = (lyricLine ?? '').replace(/\r/g, '');
	const width = Math.max(chord.length, lyric.length, 1);
	const cPad = chord.padEnd(width, ' ');
	const lPad = lyric.padEnd(width, ' ');

	const tokenRe = new RegExp(`${CHORD_PATTERN}|\\||-+`, 'g');
	const tokens: { start: number; end: number; text: string }[] = [];
	let m: RegExpExecArray | null;
	while ((m = tokenRe.exec(cPad)) !== null) {
	...
	type Group = { start: number; end: number; chord: string };
	const groups: Group[] = [];
	...
	for (const tok of tokens) {
		...
		if (/^-+$/.test(tok.text)) {
			...
			else current = { start: tok.start, end: tok.end, chord: '' };
			continue;
		}
		if (current) groups.push(current);
		current = { start: tok.start, end: tok.end, chord: tok.text };
	}
	if (current) groups.push(current);

	if (groups.length === 0) {
		return [{ chord: chord.trim(), lyric }];
	}

	const segs: ChordLyricSegment[] = [];
	if (groups[0].start > 0) {
		const head = lPad.slice(0, groups[0].start);
		if (head.trim()) segs.push({ chord: '', lyric: head });
	}
	for (let i = 0; i < groups.length; i++) {
		const start = groups[i].start;
		const end = groups[i + 1]?.start ?? width;
		segs.push({
			chord: groups[i].chord,
			lyric: lPad.slice(start, end)
		});
	}
	return segs.filter((s) => s.chord || s.lyric.trim().length > 0);
}
